...
class RokMenuLayoutFusion extends BaseRokMenuLayout {
    ...
    function renderItem(&$item, &$menu)
    {
        ...
        if (!empty($item->meta['fusion_color'][0])) {
            $color = ' '.$item->meta['fusion_color'][0];
        } else {
        	$color = ' '.$option['preset_style'];
        }
    ...
    ?>
    <li <?php if($item->hasListItemClasses()) : ?>class="<?php echo $item->getListItemClasses(); echo $color; ?>"<?php endif;?> <?php if(isset($item->css_id)):?>id="<?php echo $item->css_id;?>"<?php endif;?>>
