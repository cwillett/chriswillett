<?php 
class architect_xhtml_video_embed
{
	function format($content, $class)
	{
		preg_match('/\[youtube\](.*?)\[\/youtube\]/', $content, $url);
		if(isset($url[1]))
		{
			...
			$content = preg_replace('/\[youtube\](.*?)\[\/youtube\]/', $this->start($class).$iphonevid.$this->end($class), $content);
			return $content;
		}
		return $content;
	}
